...
ig.AnimationSheet = ig.Class.extend({
	...
	init: function( path, width, height, xmlPath ) {
		this.width = width;
		this.height = height;
		this.xmlPath = xmlPath;
		this.heroXML = new ig.HeroXML( xmlPath );
		this.image = new ig.HeroImage( path );
	}	
});
	...
	draw: function( targetX, targetY ) {
		...
		if( this.angle == 0 ) {
			this.sheet.image.drawTile(
				...
				this.flip.x, this.flip.y, this.sheet.heroXML
			);
		}
		else {
